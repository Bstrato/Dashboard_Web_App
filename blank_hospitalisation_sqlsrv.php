...
$table_name="blank_hospitalisation";

if(isset($_POST['listings']) && isset($_POST['SEARCH'])){

    $table_name=$_POST['blank_hospitalisation'];
   
   if(!empty($_POST['blank_hospitalisation'])){
...
$cluster_query = "SELECT cluster, count(cluster) as cnt2 FROM blank_hospitalisation GROUP BY cluster ORDER BY cluster";
...
$grouping = '';
$grouping_query = "SELECT groupings, count(groupings) as cnt2 FROM blank_hospitalisation GROUP BY groupings ORDER BY groupings";
...
$district = '';
$district_query ="SELECT district, count(district) as cnt3 FROM blank_hospitalisation GROUP BY district ORDER BY district";
...
$query_all="SELECT * FROM EPIMAL003_Master.dbo.blank_hospitalisation";
...
if (isset($_POST['filter_cluster'],$_POST['filter_grouping'], $_POST['filter_district']) && (!empty(trim($_POST['filter_cluster'])) && !empty(trim($_POST['filter_grouping'])) && !empty(trim($_POST['filter_district']))))
{
  ...
  $query_all="SELECT * FROM blank_hospitalisation";
...
<!-- Content -->
        ...
        <div class="content">
        <div class="buy-form">
        <h4 class="page-title" align="centre">ENHANCE PARTICIPANTS WITH MISSING HOSPITAL FORM</h4>
