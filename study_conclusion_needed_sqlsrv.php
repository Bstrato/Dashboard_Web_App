...
$table_name="conclusion_needed_datab";

if(isset($_POST['listings']) && isset($_POST['SEARCH'])){

    $table_name=$_POST['conclusion_needed_datab'];
   
   if(!empty($_POST['conclusion_needed_datab'])){
...
$cluster_query = "SELECT cluster, count(cluster) as cnt2 FROM conclusion_needed_datab GROUP BY cluster ORDER BY cluster";
...
$grouping = '';
$grouping_query = "SELECT groupings, count(groupings) as cnt2 FROM conclusion_needed_datab GROUP BY groupings ORDER BY groupings";
...
$district = '';
$district_query ="SELECT district, count(district) as cnt3 FROM conclusion_needed_datab GROUP BY district ORDER BY district";
...
$query_all="SELECT * FROM EPIMAL003_Master.dbo.conclusion_needed_datab";
...
if (isset($_POST['filter_cluster'],$_POST['filter_grouping'], $_POST['filter_district']) && (!empty(trim($_POST['filter_cluster'])) && !empty(trim($_POST['filter_grouping'])) && !empty(trim($_POST['filter_district']))))
{
  ...
  $query_all="SELECT * FROM conclusion_needed_datab";
...
<!-- Content -->
        ...
        <div class="content">
        <div class="buy-form">
        <h4 class="page-title" align="centre">STUDY CONCLUSION NEEDED - LISTINGS</h4>
...
<td> <?php echo $row['aesi'] ; ?></td>
<td> <?php echo $row['sae'] ; ?></td>
