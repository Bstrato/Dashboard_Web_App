...
$table_name="cause_of_death_unknown";

if(isset($_POST['listings']) && isset($_POST['SEARCH'])){

    $table_name=$_POST['cause_of_death_unknown'];
   
   if(!empty($_POST['cause_of_death_unknown'])){
...
$cluster_query = "SELECT cluster, count(cluster) as cnt2 FROM cause_of_death_unknown GROUP BY cluster ORDER BY cluster";
...
/*
$grouping = '';
$grouping_query = "SELECT groupings, count(groupings) as cnt2 FROM cause_of_death_unknown GROUP BY groupings ORDER BY groupings";
...
*/
$district = '';
$district_query ="SELECT district, count(district) as cnt3 FROM cause_of_death_unknown GROUP BY district ORDER BY district";
...
if (isset($_POST['filter_cluster'],$_POST['filter_grouping'], $_POST['filter_district']) && (!empty(trim($_POST['filter_cluster'])) && !empty(trim($_POST['filter_grouping'])) && !empty(trim($_POST['filter_district']))))
{
  ...
  $query_all="SELECT * FROM cause_of_death_unknown";
...
<!-- Content -->
        ...
        <div class="content">
        <div class="buy-form">
        <h4 class="page-title" align="centre">CAUSES OF DEATH UNKNOWN - LISTINGS</h4>
