...
$table_name="aefi_Rtss_opd_list_share";

if(isset($_POST['listings']) && isset($_POST['SEARCH'])){

    $table_name=$_POST['aefi_Rtss_opd_list_share'];
   
   if(!empty($_POST['aefi_Rtss_opd_list_share'])){
...
$cluster_query = "SELECT cluster, count(cluster) as cnt2 FROM aefi_Rtss_opd_list_share GROUP BY cluster ORDER BY cluster";
...
$grouping = '';
$grouping_query = "SELECT groupings, count(groupings) as cnt2 FROM aefi_Rtss_opd_list_share GROUP BY groupings ORDER BY groupings";
...
$district = '';
$district_query ="SELECT district, count(district) as cnt3 FROM aefi_Rtss_opd_list_share GROUP BY district ORDER BY district";
...
$query_all="SELECT * FROM EPIMAL003_Master.dbo.aefi_Rtss_opd_list_share";
...
if (isset($_POST['filter_cluster'],$_POST['filter_grouping'], $_POST['filter_district']) && (!empty(trim($_POST['filter_cluster'])) && !empty(trim($_POST['filter_grouping'])) && !empty(trim($_POST['filter_district']))))
{
  ...
  $query_all="SELECT * FROM aefi_Rtss_opd_list_share";
...
<!-- Content -->
        ...
        <div class="content">
        <div class="buy-form">
        <h4 class="page-title" align="centre">RTS,S OPD LISTINGS</h4>
...
<td> <?php echo $row['rvv'] ; ?></td>
<td> <?php echo $row['rr'] ; ?></td>
<td> <?php echo date_format($row['date_'],"Y-m-d H: i: s"); ?></td>
</tr>
